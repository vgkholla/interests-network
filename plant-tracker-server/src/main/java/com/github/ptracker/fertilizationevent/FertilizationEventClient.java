...
public class FertilizationEventClient implements GrpcResource.GrpcClient<String, FertilizationEvent> {
  ...
  public void create(FertilizationEvent payload, CreateRequestOptions options) {
    FertilizationEventCreateRequest request = FertilizationEventCreateRequest.newBuilder().setFertilizationEvent(payload).build();
    _blockingStub.create(request);
  }

  @Override
  public void update(FertilizationEvent payload, UpdateRequestOptions options) {
    FertilizationEventUpdateRequest request =
        FertilizationEventUpdateRequest.newBuilder().setFertilizationEvent(payload).setShouldUpsert(options.shouldUpsert()).build();
