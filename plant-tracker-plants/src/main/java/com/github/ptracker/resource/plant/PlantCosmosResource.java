...
import com.github.ptracker.common.protobuf.ProtoBufJsonInterchange;
...
public class PlantCosmosResource implements Resource<String, Plant> {
  private static final Logger LOGGER = LoggerFactory.getLogger(PlantCosmosResource.class);

  private static final String PLANT_DATABASE_NAME = "PlantsTracker";
  private static final String PLANT_CONTAINER_NAME = "plants";
  ...
  private static final ResourceResponse<Optional<Plant>> NO_MATCHING_PLANTS =
      new ResourceResponseImpl.Builder<Optional<Plant>>().payload(Optional.empty()).build();
  ...
  private static final String GET_BY_ID_QUERY = "SELECT * FROM plants where plants.id = " + ID_VARIABLE;

  private final CosmosClient _client;
  private final ProtoBufJsonInterchange<Plant, Plant.Builder> _protoBufJsonInterchange =
      new ProtoBufJsonInterchange<>(Plant::newBuilder);
  private final CosmosDBMetadataHandler _metadataHandler = new CosmosDBMetadataHandler();

  public PlantCosmosResource(CosmosClient client) {
  ...
  public ResourceResponse<Optional<Plant>> get(String key, GetRequestOptions options) {
    ...
    List<ResourceResponseImpl<Optional<Plant>>> matchingPlants =
        query.getResults(DEFAULT_QUERY_REQUEST_OPTIONS, Collections.singletonMap(ID_VARIABLE, key))
            .stream()
            .map(item -> new ResourceResponseImpl.Builder<Optional<Plant>>().payload(
                Optional.of(_protoBufJsonInterchange.convert(item)))
                .metadata(_metadataHandler.getStorageMetadata(item))
                .build())
            .collect(Collectors.toList());
    return matchingPlants.isEmpty() ? NO_MATCHING_PLANTS : Iterables.getOnlyElement(matchingPlants);
  }
  ...
  public ResourceResponse<Void> create(Plant payload, CreateRequestOptions options) {
    ...
    PlantUtils.verifyPlant(payload);
    ...
    try {
      ...
      LOGGER.error("Item: {}, ID {}", item, payload.getId());
      throw e;
    }
  }
  ...
  public ResourceResponse<Void> delete(String key, DeleteRequestOptions options) {
    ...
    ObjectNode objectNode = _protoBufJsonInterchange.convert(Plant.newBuilder().setId(key).build());
  ...
  private CosmosContainer getContainer() {
    return _client.getDatabase(PLANT_DATABASE_NAME).getContainer(PLANT_CONTAINER_NAME);
  }
}
